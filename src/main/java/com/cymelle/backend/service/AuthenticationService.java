...
import com.cymelle.backend.repository.UserRepository;
import com.cymelle.backend.security.JwtService;
...
public class AuthenticationService {
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        var user = repository.findByEmail(request.getEmail())
                .orElseThrow(() -> new com.cymelle.backend.exception.ResourceNotFoundException("User not found: " + request.getEmail()));
        var jwtToken = jwtService.generateToken(user);
        return AuthenticationResponse.builder()
                .token(jwtToken)
                .build();
